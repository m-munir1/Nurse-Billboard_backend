...
const {account} = require('../../../../helpers/validation_schema/admin/human_resources/account');


app.get("/",verifyTokenAndPermission(["human_resources"]),(req,res)=>{
    
    pool.query(`
    SELECT * FROM human_resources WHERE phone LIKE '${req.user.phone}';`,(err,rows,fields) =>{
        if(err){   
...
app.put("/",verifyTokenAndPermission(["human_resources"]),(req,res)=>{
